...
export default {
  load(): MembersData {
    const filePath = path.join(__dirname, 'members-data.md')
    ...
    yamlBlocks.forEach(block => {
      ...
      try {
        ...
        Object.keys(parsed).forEach(key => {
          if (key in result) {
            result[key as keyof MembersData] = parsed[key] || []
          }
        })
      } catch (error) {
...
// 导出类型定义供其他文件使用
export type { MembersData, TeamMember, SocialLink, SocialLinkIcon }